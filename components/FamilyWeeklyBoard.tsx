...
import type { TodoItem, CalendarEvent } from '@/lib/types';
import { DAYS, getWeekNumber, getYear, goalDayToISO, todayISO } from '@/lib/utils';

const DAY_ACCENT: Record<string, { color: string; headerBg: string; label: string }> = {
...
interface Props {
  ...
  onQuickAdd?: (text: string, dates: string[]) => void;
}

export default function FamilyWeeklyBoard({
  allTodos, events, onAdd, onToggle, onDelete, onGoogleToggle, onQuickAdd,
}: Props) {
  ...
  const [quickDays,   setQuickDays]   = useState<Set<string>>(
    () => new Set([(DAYS as string[])[(new Date().getDay() + 6) % 7]])
  );
  ...
  const viewStartStr = toLocalISO(weekStart);
  const viewEndStr   = toLocalISO(viewWeekEnd);
  const fmtWD        = (d: Date)  => d.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
  const weekLabel    = `Week of ${fmtWD(weekStart)} – ${fmtWD(viewWeekEnd)}, ${viewYear}`;

  const weekDates: Record<string, string> = {};
  (DAYS as string[]).forEach(day => { weekDates[day] = goalDayToISO(viewWeek, viewYear, day); });

  const weekTodos  = allTodos.filter(t => t.date >= viewStartStr && t.date <= viewEndStr);
  const weekEvents = events.filter(e => e.source === 'google' && e.date >= viewStartStr && e.date <= viewEndStr);

  const totalTasks = weekTodos.length + weekEvents.length;
  const doneTasks  = weekTodos.filter(t => t.completed).length + weekEvents.filter(e => !!e.completed).length;
  ...
  function toggleQuickDay(day: string) {
    setQuickDays(prev => {
  ...
  async function handleQuickAdd(e: React.FormEvent) {
    ...
    setQuickAdding(true);
    try {
      await onQuickAdd(text, Array.from(quickDays).map(d => weekDates[d]));
      setQuickText('');
    } finally {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {dayItems.map(item =>
                    item.kind === 'todo' ? (
                      <TaskRow key={item.data.id} hoverBg={a.headerBg} completed={item.data.completed}>
                        <CircleCheckbox
        ...
        <div className="hidden xl:flex w-56 shrink-0 flex-col gap-3">
          ...
          >
            ...
            <div className="flex items-end justify-between mb-4">
              {(DAYS as string[]).map(day => {
                const dt = weekDates[day];
                const t  = weekTodos.filter(x => x.date === dt).length + weekEvents.filter(e => e.date === dt).length;
                const d  = weekTodos.filter(x => x.date === dt && x.completed).length + weekEvents.filter(e => e.date === dt && !!e.completed).length;
                const a  = DAY_ACCENT[day] ?? DAY_ACCENT.Mon;
                return (
                  <div key={day} className="flex flex-col items-center gap-1">
                    <span
                      className="h-2.5 w-2.5 rounded-full"
                      style={{ background: t === 0 ? '#E4E4E7' : d === t ? a.color : `${a.color}55` }}
                      title={`${a.label}: ${d}/${t}`}
                    />
            ...
            <form onSubmit={handleQuickAdd} className="space-y-2.5">
              ...
              <div className="flex items-center justify-between">
                {(DAYS as string[]).map(d => {
                  ...
                  const a   = DAY_ACCENT[d] ?? DAY_ACCENT.Mon;
                  return (
                    <button
                      key={d}
                      type="button"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              ...
              {(DAYS as string[]).map(day => {
                const a        = DAY_ACCENT[day] ?? DAY_ACCENT.Mon;
                const dt       = weekDates[day];
                const dayTodos = weekTodos.filter(t => t.date === dt);
                const dayGoog  = weekEvents.filter(e => e.date === dt);
                const t   = dayTodos.length + dayGoog.length;
                const d   = dayTodos.filter(t => t.completed).length + dayGoog.filter(e => !!e.completed).length;
...
function CircleCheckbox({ checked, color, onChange }: { checked: boolean; color: string; onChange: () => void }) {
  return (
      ...
      <svg viewBox="0 0 16 16" className="h-full w-full" aria-hidden="true">
        {checked ? (
          <>
            ...
            <path d="M5 8l2 2 4-4" stroke="white" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
          </>
        ) : (
