...
type Result = { status: 'sent' | 'not_found' | 'send_error'; error?: string } | null;
...
export default function ForgotPasswordPage() {
  ...
  const subtitle =
    result?.status === 'sent'       ? 'Check your inbox' :
    result?.status === 'not_found'  ? 'Email not found' :
    result?.status === 'send_error' ? 'Something went wrong' :
        ...
        >
          {result?.status === 'sent' ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-xs text-stone-400">The link expires in 1 hour.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              {result?.status === 'not_found' && (
                <div className="flex items-start gap-3 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                  ...
                  <Link href="/register" className="underline font-semibold">Create one?</Link>
                </div>
              )}
              ...
              {result?.status === 'send_error' && (
                <div className="flex items-start gap-3 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">
                  ...
                  {result.error ?? 'Failed to send the reset email. Please try again.'}
                </div>
              )}
