  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <button onClick={prevWeek} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 transition">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 16 16">
            <path d="M10 4L6 8l4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
        <div className="text-center">
          <p className="text-sm font-semibold text-slate-700">Week {week}, {year}</p>
          {isCurrentWeek && <span className="text-xs text-indigo-600 font-medium">This Week</span>}
        </div>
        <button onClick={nextWeek} className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 transition">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 16 16">
        ...
        <table className="w-full min-w-[600px] text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="w-28 px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Meal</th>
              {DAYS.map(day => (
                <th key={day} className="px-2 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">
                  {day}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {MEAL_TYPES.map(({ key, label }, rowIdx) => (
              <tr key={key} className={rowIdx < MEAL_TYPES.length - 1 ? 'border-b border-slate-50' : ''}>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                    key === 'breakfast' ? 'bg-amber-50 text-amber-700' :
                    key === 'lunch'     ? 'bg-sky-50 text-sky-700' :
                    ...
                    <td key={day} className="px-2 py-2 text-center">
                      {isActive ? (
                        <input
                          ...
                          className="w-full rounded-lg border border-indigo-300 bg-white px-2 py-1 text-center text-xs text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100"
                        />
                      ) : (
                        <button
      ...
      <p className="text-xs text-slate-400 text-center">Click any cell to edit</p>
    </div>
  );
}
