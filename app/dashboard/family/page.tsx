        ...
        <>
          ...
          <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-sm flex-col bg-white shadow-2xl">
            ...
            <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4">
              <h2 className="text-base font-bold text-stone-800">Family Info ℹ️</h2>
              <button
                onClick={() => setInfoOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-7">
              ...
              <PanelSection label={`Members · ${members.length}`}>
                <ul className="space-y-2">
                  {members.map(m => {
                    ...
                    return (
                      <li key={m.id} className="flex items-center gap-3 rounded-xl border border-stone-100 bg-stone-50 px-3 py-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-stone-800 leading-none">
                            {m.name}
                            {isMe && <span className="ml-2 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-700">You</span>}
                          </p>
              ...
              <PanelSection label="Emergency Contacts">
                {contacts.length > 0 && (
                  <ul className="space-y-2 mb-3">
                    {contacts.map(c => (
                      <li key={c.id} className="flex items-center gap-3 rounded-xl border border-stone-100 bg-stone-50 px-3 py-2.5">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-red-50 text-base">🚨</div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-stone-500 mt-0.5">
                            ...
                            {c.phone && <a href={`tel:${c.phone}`} className="font-medium text-red-700 hover:underline">{c.phone}</a>}
                          </p>
                        </div>
                        <button
                ...
                {addingContact ? (
                  <div className="rounded-xl border border-stone-200 bg-stone-50 p-3 space-y-2">
                    <p className="text-xs font-semibold text-stone-600">New contact</p>
                    {(['name', 'relationship', 'phone'] as const).map((field, i) => (
                      <input
                        key={field}
                        autoFocus={i === 0}
                        placeholder={field === 'name' ? 'Full name *' : field === 'relationship' ? 'Relationship' : 'Phone'}
                        value={contactForm[field]}
                        onChange={e => setContactForm(p => ({ ...p, [field]: e.target.value }))}
                        className="w-full rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm text-stone-800 placeholder-stone-400 outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100"
                      />
                    ))}
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 rounded-xl border border-dashed border-stone-300 px-3 py-2.5 text-sm font-medium text-stone-500 hover:border-red-300 hover:text-red-700 hover:bg-red-50 transition w-full"
                  >
              ...
              <PanelSection label="Invite Code">
                <p className="mb-3 text-xs text-stone-500">Share this code to invite people to <span className="font-semibold text-stone-700">{family.name}</span>.</p>
                <div className="flex items-center gap-3 rounded-xl border border-stone-200 bg-stone-50 px-4 py-3">
                  <span>🔑</span>
                  <span className="flex-1 font-mono text-xl font-bold tracking-[0.3em] text-red-800">{family.code}</span>
                  <button
                    onClick={() => { navigator.clipboard.writeText(family.code); setCodeCopied(true); setTimeout(() => setCodeCopied(false), 2000); }}
                    className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-semibold transition-all ${codeCopied ? 'bg-emerald-100 text-emerald-800' : 'bg-stone-200 text-stone-700 hover:bg-stone-300'}`}
                  >
